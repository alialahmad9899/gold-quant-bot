...
class FakeBot:
    def __init__(self, state_file):
        ...
        def generate_quant_signal():
            ...
            return {
                "status": "SIGNAL", "type": "🟢 شراء مرن", "entry": 3000.0, "sl": 2990.0,
                "tp1": 3020.0, "tp2": 3040.0, "confidence": 82, "rsi": 58.0,
                "dxy_corr": -0.70, "candle_id": "XAUUSD_M15_TEST",
                "smc_note": "bullish BOS + bullish FVG + sweep", "score_bull": 5.5,
                "score_bear": 2.0, "gemini_note": "approved",
            }
        ...
        def get_market_data():
            return {"gold": self.market_price, "price_feed": {"status": "ACTIVE", "mid": self.market_price}}

        self.get_market_data = get_market_data

        def analyze_institutional_engine():
            return {"h4_trend": self.h4_trend, "state_label": self.state_label, "smc": dict(self.smc)}
...
def test_dynamic_management_is_exposed_on_bot_cache(tmp_path, monkeypatch):
    ...
    assert bot.GLOBAL_CACHE["trade_lawyer"]["action"] == "PROTECT_PROFIT"
...
def test_invalidation_and_reversal_are_derived_from_existing_market_analysis(tmp_path, monkeypatch):
    ...
    assert result["lawyer"]["action"] == "EXIT"


def test_institutional_gate_does_not_hard_block_moderate_rr(tmp_path, monkeypatch):
    monkeypatch.setenv("INSTITUTIONAL_GEMINI_REVIEW", "0")
    bot = FakeBot(tmp_path / "unused.json")
    manager = TradeIntelligence(TradeStateStore(str(tmp_path / "state.json")))
    bridge = Phase2RuntimeIntegration(bot, manager).install()
    ...
    def moderate_rr():
        return {
            "status": "SIGNAL", "type": "🟢 شراء مرن", "entry": 3000.0, "sl": 2990.0,
            "tp1": 3005.0, "tp2": 3010.0, "confidence": 80, "rsi": 58.0,
            "dxy_corr": -0.70, "candle_id": "MODERATE_RR", "smc_note": "bullish BOS + bullish FVG",
        }

    bridge._original_generate = moderate_rr
    result = bridge._wrapped_generate()
    assert result["status"] == "SIGNAL"
    assert manager.has_active_trade()
    assert result["risk_decision"] in {"APPROVE", "APPROVE_WITH_CAUTION"}
