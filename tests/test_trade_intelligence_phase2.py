...
def test_active_position_lock(tmp_path):
    engine = TradeIntelligence(TradeStateStore(str(tmp_path / "trade_state.json")))
...
def test_invalidation_reversal_review(tmp_path):
    engine = TradeIntelligence(TradeStateStore(str(tmp_path / "trade_state.json")))
...
def test_close_lifecycle(tmp_path):
    engine = TradeIntelligence(TradeStateStore(str(tmp_path / "trade_state.json")))
    engine.open_trade(TradeThesis(direction="BUY", entry=2300))
...
def test_manage_returns_dynamic_trade_controls_after_tp1(tmp_path):
    engine = TradeIntelligence(TradeStateStore(str(tmp_path / "trade_state.json")))
    engine.open_trade(TradeThesis(direction="BUY", entry=2300))
...
def test_reversal_has_priority_over_temporary_pressure(tmp_path):
    engine = TradeIntelligence(TradeStateStore(str(tmp_path / "trade_state.json")))
