...
Historical lessons marked HIGH and explicitly matching the active setup remain
hard risk vetoes. RR/confidence/regime mismatch remain quality signals, and
Gemini is advisory unless INSTITUTIONAL_AI_VETO=1.
"""
...
def _lesson_applies(direction:str,lesson:str,context:str)->bool:
    ...
    # High-severity lessons are applied when they explicitly target this direction
    # and at least one setup condition is present; this preserves their hard-veto role.
    if severity=="HIGH":
        explicit_dir=(direction=="BUY" and any(k in low for k in ("buy","شراء"))) or (direction=="SELL" and any(k in low for k in ("sell","بيع")))
        if explicit_dir:
        ...
        return True if len(re.findall(r"[\w\u0600-\u06ff]+",low))<=4 else False
...
def review_trade(signal_data:dict[str,Any],market_summary:dict[str,Any],lessons:list[Any]|None=None,smc:dict[str,Any]|None=None)->TradeReviewResult:
    ...
    entry_score=15 if supporting_smc and rr is not None and rr>=MIN_RR else 11 if supporting_smc else 8; rr_score=min(15,max(3,int((rr or 0)/3.0*15))) if rr is not None else 0; confidence_score=min(10,max(2,int(confidence/10))); confidence_score=max(2,confidence_score-1) if confidence<MIN_CONFIDENCE else confidence_score; momentum_score=10 if rsi is None else (8 if (direction=="BUY" and rsi<70) or (direction=="SELL" and rsi>30) else 5); liquidity_score=10 if any(_bool(smc.get(k)) for k in ("liquidity_bullish","liquidity_bearish","sweep_bullish","sweep_bearish")) else 5 if opposing_smc else 3; regime_score=5 if regime in {"TRENDING_BULLISH","TRENDING_BEARISH"} and ((direction=="BUY" and h4=="BULLISH") or (direction=="SELL" and h4=="BEARISH")) else 3
...
def build_adversarial_prompt(signal_data,market_summary,deterministic,lessons):
    return f"أنت مدير مخاطر كمي مرن متخصص في XAU/USD. افترض أن الصفقة قد تفشل وابحث عن الخطر الحقيقي، لكن لا ترفضها لمجرد أنها غير مثالية. لا تتجاوز hard vetoes الحتمية. أرجع JSON فقط. التقييم الحتمي: {json.dumps(deterministic.to_dict(),ensure_ascii=False,default=str)} الدروس: {chr(10).join('- '+str(x) for x in lessons) if lessons else '- لا توجد دروس'} الإشارة: {json.dumps(signal_data,ensure_ascii=False,default=str)} السوق: {json.dumps(market_summary,ensure_ascii=False,default=str)} {{"approved":true,"decision":"APPROVE","reason":"سبب عربي مختصر","thesis":"ملخص","invalidation":"شرط البطلان","reversal":"شرط الانعكاس"}}"
...
def apply_ai_review(deterministic,raw_ai):
    ...
    if not ai_approved or ai_decision in {"REJECT","REVERSE"}:
        if AI_VETO_ENABLED: deterministic.approved=False; deterministic.decision="REJECT"; deterministic.reason=str(ai.get("reason") or "مراجعة Gemini رأت خطراً إضافياً.")
            ...
            deterministic.approved=deterministic.approved
            # Keep deterministic decision unchanged when AI is advisory.
        ...
        elif deterministic.approved and deterministic.decision=="APPROVE": deterministic.decision="APPROVE"
