...
def patch_lawyer(bot):
    def snapshot():
        ...
        status="تأكد سعرياً" if conflict["confirmed"] else "لم يتأكد سعرياً بالكامل"
        return base+f"\n\n📰 تحديث الأخبار: خبر مؤثر ضد الصفقة ({conflict['title']}) بتأثير {conflict['impact']}/100.\nالحكم المعدّل: {conflict['action']}\nالخبر {status}؛ لا يوجد خروج تلقائي من العنوان وحده."
...
def patch_health(bot):
    ...
    async def health(update,context):
        try:
            ...
            d=decision_layer.health(); cache=getattr(bot,"GLOBAL_CACHE",{}) or {}; nh=cache.get("news_health") or {}; bal=cache.get("direction_balance")
            balance_text=f"BUY {bal.get('buy_pct')}% / SELL {bal.get('sell_pct')}%" if isinstance(bal,dict) else "لا توجد عينة بعد"
            text=(f"🧠 صحة القرار: {'🟢 تعمل' if d.get('patched') else '🟠 قيد التثبيت'}\n"
                  ...
                  f"📐 توازن الاتجاه: {balance_text}")
            await bot.safe_reply_text(update,text,reply_markup=bot.get_main_keyboard())
        except Exception: pass
