...
The previous implementation could veto a signal a second time after the
institutional reviewer had already made the final decision. This version keeps
hard safety only for invalid live data/levels and delegates the final trade
policy to decision_layer.py. Gemini is advisory by default.
"""
...
def _is_pg(bot,conn):
    try: return bool(getattr(bot,"is_postgres",lambda:False)() and psycopg2 is not None and isinstance(conn,psycopg2.extensions.connection))
...
def _patch_gemini(bot):
    ...
    def advisory(signal_data,market_summary):
        ...
        # Canonical policy: Gemini disagreement is advisory unless explicitly promoted to a hard veto.
        if GEMINI_HARD_VETO:
...
def _patch_log_trade(bot):
    ...
    def guarded(*args,**kwargs):
        # log_trade arguments in the current bot: direction, entry, sl, tp1, tp2, rsi, dxy_corr, macd, stoch, volatility, confidence, candle_id=...
        vals=list(args)
        direction=str(vals[0] if len(vals)>0 else kwargs.get("signal_type") or "").upper(); entry=vals[1] if len(vals)>1 else kwargs.get("entry_price"); sl=vals[2] if len(vals)>2 else kwargs.get("sl"); tp1=vals[3] if len(vals)>3 else kwargs.get("tp1"); confidence=vals[10] if len(vals)>10 else kwargs.get("confidence",1.0)
        try:
            entry=float(entry); sl=float(sl); tp1=float(tp1); confidence=float(confidence)
            risk=abs(entry-sl); reward=(tp1-entry) if "BUY" in direction or "شراء" in direction else (entry-tp1); rr=reward/risk if risk>0 else 0.0
            if confidence<MIN_CONFIDENCE: LOGGER.info("[SAFETY] Low-confidence candidate blocked from trade ledger: %.2f",confidence); return False,None
            if rr<MIN_RR: LOGGER.info("[SAFETY] RR below minimum blocked from trade ledger: %.2f",rr); return False,None
            if risk/entry<MIN_STOP_PCT: LOGGER.info("[SAFETY] Stop distance too tight blocked from trade ledger: %.5f",risk/entry); return False,None
        except Exception:
            return False,None
...
def install_signal_safety(bot:Any|None=None):
    ...
    with _LOCK:
        ...
        try:
            ...
            bot._decision_layer=decision_layer; decision_layer.install(bot)
        except Exception as exc:
            LOGGER.exception("❌ Decision layer installation failed: %s",exc)
        _INSTALLED=True
        LOGGER.info("✅ Canonical safety installed: Gemini advisory by default; decision_layer owns final policy.")
