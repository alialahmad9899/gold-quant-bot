...
import logging, math, os, threading
...
def _db_event(bot,result,live_price,gap_pct):
    conn=None
    try:
        conn=bot.get_db_connection(); cur=conn.cursor(); pg=_is_pg(bot,conn); sql="INSERT INTO runtime_decision_events(candle_id,direction,final_decision,decision_state,confidence,entry,live_price,price_gap_pct,ai_advisory) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)" if pg else "INSERT INTO runtime_decision_events(candle_id,direction,final_decision,decision_state,confidence,entry,live_price,price_gap_pct,ai_advisory) VALUES (?,?,?,?,?,?,?,?,?)"
        raw=str(result.get("type") or ""); direction="BUY" if "شراء" in raw else "SELL" if "بيع" in raw else str(result.get("direction") or "UNKNOWN")
        cur.execute(sql,(str(result.get("candle_id") or ""),direction,str(result.get("final_decision") or ""),str(result.get("decision_state") or ""),float(result.get("confidence") or 0.0),float(result.get("entry") or 0.0),live_price,gap_pct,bool(result.get("ai_advisory")))); conn.commit()
    except Exception as exc:
...
def _live_price(bot):
    try:
        ...
        if quote and math.isfinite(float(quote.get("price"))): return float(quote["price"])
    except Exception: pass
    try:
        market=bot.get_market_data() or {}; value=(market.get("price_feed") or {}).get("mid") or market.get("gold"); value=float(value) if value is not None else None
...
def _patch_generation(bot):
    ...
    def wrapped(*args,**kwargs):
        ...
        result["decision_state"]="TRADE_READY"; result["candidate"]=True
        _db_event(bot,result,result.get("live_price_at_decision"),result.get("entry_price_gap_pct"))
        return result
...
def _lawyer_snapshot(bot):
    ...
    try:
        ...
        return f"🧑‍⚖️ تعذر تحديث المحامي حالياً: {type(exc).__name__}: {exc}"
    finally:
        if conn is not None:
            try: bot.release_db_connection(conn)
            except Exception: pass
...
def _install_application_router(bot):
    ...
    try: from telegram.ext import Application
        ...
        LOGGER.warning("Telegram Application unavailable: %s",exc); return
    with _LOCK:
        ...
        async def process_update(app,update):
            ...
            try:
                ...
                if text in {"📰 أخبار الذهب","/news"}:
                    if not _authorized(bot,update): await message.reply_text("🔒 يرجى إدخال كلمة السر أولاً."); return
                    from decision_layer import _news_snapshot
                    await message.reply_text(await _news_snapshot(bot),reply_markup=bot.get_main_keyboard()); return
                if text in {"📈 إحصائيات النظام","/stats"}:
                    if not _authorized(bot,update): await message.reply_text("🔒 يرجى إدخال كلمة السر أولاً."); return
                    await message.reply_text(_stats_message(bot),reply_markup=bot.get_main_keyboard()); return
            except Exception as exc:
                LOGGER.exception("Telegram production routing failed: %s",exc)
...
def install(bot):
    ...
    with _LOCK:
        ...
        _INSTALLED=True
    try:
        _ensure_schema(bot); _keyboard_patch(bot); _patch_generation(bot); _install_application_router(bot)
