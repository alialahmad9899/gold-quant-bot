...
# Temporary CI-debug script retained only until the final repair commit removes this helper.
...
"""
    if old not in s: raise RuntimeError('feed-state anchor not found')
...
'''
    if old_cache_fetch not in s: raise RuntimeError('background Yahoo fetch block not found')
    ...
    if old_price not in s: raise RuntimeError('/price block not found')
    s=s.replace(old_price,new_price,1)
    for forbidden in ('PAXGUSDT','GC=F','ifcmarkets.net','IFC Markets','metals.dev'):
        if forbidden.lower() in s.lower(): raise RuntimeError(f'forbidden source remains: {forbidden}')
    if 'yf.download(' in s or 'import yfinance as yf' in s: raise RuntimeError('yfinance fallback remains')
    if 'https://api.argentapi.com/v1/spot/gold' not in s or "headers={'X-API-Key': ARGENT_API_KEY" not in s: raise RuntimeError('ArgentAPI canonical request missing')
...
def run_tests():
    ...
    wanted={'_parse_price_feed_timestamp','_finite_positive','_build_canonical_xauusd_feed','_refresh_cached_feed_status','_sanitize_feed_error','_set_argent_cooldown','_argent_cooldown_active','_build_missing_argent_feed','_argentapi_error_message','_log_argentapi_failure','fetch_canonical_xauusd_feed','get_xauusd_execution_price','_set_yahoo_cooldown','_yahoo_cooldown_active','fetch_yahoo_direct','get_chart_data_cached','_trade_direction','validate_trade_levels','_calculate_realized_r','evaluate_trade_lifecycle'}
    ...
    class Resp:
        def __init__(self,status,payload=None,text=''): self.status_code=status; self._payload=payload; self.text=text
        def json(self):
    ...
    class FakeRequests:
        ...
        def get(self,url,headers=None,timeout=None):
            self.calls+=1; self.seen.append((url,dict(headers or {}),timeout)); item=self.responses.pop(0)
            if isinstance(item,BaseException): raise item
            return item
    class FakeNP: isfinite=staticmethod(math.isfinite)
    now=datetime.now(timezone.utc)
    base={'bid':4395.0,'ask':4397.0,'mid':4396.0,'price':4396.0,'sourceTimestamp':now.isoformat(),'fetchedAt':int(now.timestamp()*1000),'ageMs':10000,'stale':False}
    ns={'np':FakeNP,'math':math,'datetime':datetime,'timezone':timezone,'timedelta':timedelta,'time':time,'threading':threading,'logger':FakeLogger(),'ARGENT_API_KEY':'test-key','PRICE_FEED_STALE_SECONDS':90,'PRICE_FEED_MIN_REQUEST_INTERVAL':60.0,'ARGENT_AUTH_COOLDOWN_SECONDS':600,'ARGENT_TRANSIENT_COOLDOWN_SECONDS':300,'ARGENT_FEED_STATE':{'blocked_until':0.0,'error_type':None,'error_message':None},'PRICE_FEED_LOCK':threading.Lock(),'CANONICAL_XAUUSD_FEED_CACHE':{'feed':None,'last_request_monotonic':0.0},'requests':None,'pd':__import__('pandas'),'YAHOO_COOLDOWN_SECONDS':1800,'YAHOO_FETCH_INTERVAL_SECONDS':900,'YAHOO_FEED_STATE':{'blocked_until':0.0,'error_type':None,'error_message':None},'YAHOO_STATE_LOCK':threading.Lock()}
    exec(compile(ast.Module(body=nodes,type_ignores=[]),'bot.py','exec'),ns)
    build=ns['_build_canonical_xauusd_feed']; fetch=ns['fetch_canonical_xauusd_feed']; exec_price=ns['get_xauusd_execution_price']
    feed=build(base,fetched_at=now); assert feed['status']=='ACTIVE' and feed['mid']==4396.0
    ...
    stale=build(dict(base,ageMs=120000,sourceTimestamp=(now-timedelta(seconds=120)).isoformat()),fetched_at=now); assert stale['status']=='STALE' and exec_price(stale,'BUY','ENTRY') is None
    assert build({'sourceTimestamp':now.isoformat()},fetched_at=now)['status']=='MISSING'
    def run_fetch(response,key='test-key'):
        ns['CANONICAL_XAUUSD_FEED_CACHE']={'feed':None,'last_request_monotonic':0.0}; ns['ARGENT_FEED_STATE']={'blocked_until':0.0,'error_type':None,'error_message':None}; ns['ARGENT_API_KEY']=key
        fake=FakeRequests([response]); ns['requests']=fake; return fetch(),fake
    result,fake=run_fetch(Resp(200,base)); print('DEBUG_VALID_ARGENT',result,'calls=',fake.calls,'seen=',fake.seen); assert result['status']=='ACTIVE' and fake.calls==1
    assert fake.seen[0][0]=='https://api.argentapi.com/v1/spot/gold' and fake.seen[0][1]['X-API-Key']=='test-key' and 'test-key' not in fake.seen[0][0]
    assert fetch()['status']=='ACTIVE' and fake.calls==1
    for code in (401,403):
        result,fake=run_fetch(Resp(code,{'message':'invalid_key'})); assert result['status']=='MISSING' and result['error_type']=='authentication'; assert fetch()['status']=='MISSING' and fake.calls==1
    ...
    src=BOT.read_text(encoding='utf-8'); assert 'trade_outcome:{trade_id}' in src; assert "enqueue_learning_event('TRADE_OUTCOME'" in src; assert 'evaluate_trade_lifecycle(sig_type,current_status,entry,sl,tp1,tp2' in src; assert 'slippage=NULL' in src; assert 'yf.download(' not in src and 'import yfinance as yf' not in src
    for forbidden in ('PAXGUSDT','GC=F','ifcmarkets.net','IFC Markets','metals.dev'): assert forbidden.lower() not in src.lower()
    print('MARKET_FEED_AND_PHASE1_TESTS_OK')

if __name__=='__main__': patch_bot(); run_tests(); print('--- FINAL DIFF ---'); subprocess.run(['git','diff','--','bot.py'],check=True); subprocess.run(['git','diff','--stat','--','bot.py'],check=True)
