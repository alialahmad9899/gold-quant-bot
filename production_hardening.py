...
_INSTALL_LOCK = threading.Lock(); _INSTALLED = False
...
def _event_key(title: str, published_at: datetime) -> str:
    tokens = re.findall(r"[a-z0-9]+", _norm(title))[:18]; bucket = int(published_at.timestamp() // NEWS_EVENT_CLUSTER_SECONDS)
    return hashlib.sha256((" ".join(tokens) + f"|{bucket}").encode()).hexdigest()[:16]


def parse_economic_surprise(text: str) -> dict[str, float | None]:
    text = str(text or ""); patterns = {"actual": r"(?:actual|reported|released)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\s*%?", "forecast": r"(?:forecast|expected|estimate)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\s*%?", "previous": r"(?:previous|prior)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\s*%?"}
...
def semantic_news_signal(title: str, summary: str = "") -> dict[str, Any]:
    ...
    positive = ("rate cut", "cuts rates", "dovish", "easing", "lower rates", "weaker dollar", "dollar falls", "falling yields", "safe haven", "gold rises", "gold gains", "inflation rises", "war escalates", "escalation", "sanction")
    negative = ("rate hike", "hikes rates", "hawkish", "higher rates", "strong dollar", "dollar rises", "rising yields", "yield rises", "gold falls", "gold drops", "inflation cools", "ceasefire", "dovish expectations fade")
    bull = sum(1 for p in positive if p in text); bear = sum(1 for p in negative if p in text); surprise = parse_economic_surprise(text)
    if surprise.get("surprise") is not None:
        macro = any(k in text for k in ("cpi", "inflation", "nfp", "payroll", "jobs", "employment", "pce")); s = float(surprise["surprise"] or 0)
        if macro and s > 0: bear += 2
        elif macro and s < 0: bull += 2
    direction = "NEUTRAL" if bull == bear else "BULLISH_GOLD" if bull > bear else "BEARISH_GOLD"
    direct = any(k in text for k in ("gold", "xau", "bullion")); high = any(k in text for k in ("fomc", "fed", "powell", "cpi", "pce", "nfp", "payroll", "rate decision", "emergency"))
    impact = min(100, 25 + 20 * int(direct) + 15 * int(high) + 15 * min(3, abs(bull - bear))); impact = min(impact, 30) if direction == "NEUTRAL" else impact
    confidence = min(95, 45 + 10 * int(direct) + 10 * int(high) + 10 * min(3, abs(bull - bear)) + 10 * int(surprise.get("surprise") is not None))
    return {"direction": direction, "impact": impact, "confidence": confidence, "surprise": surprise, "material": bool(direction != "NEUTRAL" and impact >= 45)}
...
def price_reaction_from_event(event_time: datetime, now_price: float, price_history: list[tuple[datetime, float]]) -> dict[str, Any]:
    ...
    change_pct = (now_price - baseline) / baseline * 100.0; direction = "UP" if change_pct > 0 else "DOWN" if change_pct < 0 else "FLAT"
...
def _ensure_tables(bot: Any) -> None:
    ...
    try:
        ...
        if pg:
            cur.execute("CREATE TABLE IF NOT EXISTS direction_calibration (id BIGSERIAL PRIMARY KEY, direction TEXT NOT NULL, outcome TEXT NOT NULL, candle_id TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_direction_calibration_created ON direction_calibration(created_at)")
            cur.execute("CREATE TABLE IF NOT EXISTS news_events (event_id TEXT PRIMARY KEY, title TEXT, source TEXT, published_at TEXT, direction TEXT, impact INTEGER, confidence INTEGER, surprise DOUBLE PRECISION, source_count INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
...
def record_direction(bot: Any, direction: str, outcome: str, candle_id: str = "") -> None:
    ...
    cache = getattr(bot, "GLOBAL_CACHE", None); history = list(cache.get("direction_calibration") or []) if isinstance(cache, dict) else []
    history.append({"direction": direction, "outcome": outcome, "candle_id": candle_id, "timestamp": _now().isoformat()}); history = history[-DIRECTION_WINDOW:]
    if isinstance(cache, dict):
        cache["direction_calibration"] = history; total = len(history); buys = sum(x["direction"] == "BUY" for x in history); sells = total - buys
...
def _patch_signal_safety() -> None:
    try:
        ...
        if HARD_GEMINI_VETO: return
        original = signal_safety._normalize_vetting_result
        if getattr(original, "_production_hardening", False): return
        def advisory(raw: Any) -> dict[str, Any]:
            result = original(raw)
            if not result.get("approved", False):
                result["approved"] = True; result["advisory"] = True; result["reason"] = f"مراجعة Gemini تحفظية فقط: {result.get('reason') or 'لا توجد موافقة AI'}"
            return result
        advisory._production_hardening = True; signal_safety._normalize_vetting_result = advisory
        LOGGER.info("🧠 Gemini signal veto switched to advisory mode; institutional hard veto remains canonical.")
    except Exception as exc: LOGGER.debug("signal-safety hardening deferred: %s", exc)
...
def _patch_news_module(bot: Any) -> None:
    ...
    except Exception as exc: LOGGER.debug("news module hardening deferred: %s", exc)
...
def install(bot: Any) -> None:
    ...
    with _INSTALL_LOCK:
        ...
        _ensure_tables(bot); _patch_signal_safety(); _patch_news_module(bot)
        cache = getattr(bot, "GLOBAL_CACHE", None)
        if isinstance(cache, dict): cache.setdefault("production_hardening", {}); cache["production_hardening"].update({"installed": True, "installed_at": _now().isoformat(), "gemini_hard_veto": HARD_GEMINI_VETO, "news_reaction_window_seconds": NEWS_REACTION_WINDOW_SECONDS, "direction_window": DIRECTION_WINDOW})
        _INSTALLED = True; LOGGER.info("✅ Production hardening installed.")
