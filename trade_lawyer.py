...
class TradeLawyerDecision:
    decision: str; score: int; reasons: list[str]; warnings: list[str]
...
class ActiveTradeLawyerAdvice:
    action: str; urgency: str; confidence: int; reason: str; thesis_status: str; recommended_sl: float|None=None; recommended_tp1: float|None=None; recommended_tp2: float|None=None; add_condition: str=""; avoid_condition: str=""; ai_used: bool=False; ai_model: str=""
    def to_dict(self): return asdict(self)

AI_INTERVAL_SECONDS=float(os.getenv("TRADE_LAWYER_AI_INTERVAL_SECONDS","900")); ADD_SCORE_THRESHOLD=int(os.getenv("TRADE_LAWYER_MIN_ADD_SCORE","68")); LAWYER_NOTIFICATION_MIN_SECONDS=float(os.getenv("TRADE_LAWYER_NOTIFICATION_MIN_SECONDS","120")); NEWS_NOTIFICATION_MIN_SECONDS=float(os.getenv("NEWS_NOTIFICATION_MIN_SECONDS","120"))

def _direction(value):
    raw=str(value or "").upper(); return "BUY" if "BUY" in raw or "شراء" in raw else "SELL" if "SELL" in raw or "بيع" in raw else "UNKNOWN"
def _r_multiple(direction,entry,price,sl): return None if sl is None or abs(entry-sl)<=0 else (price-entry if direction=="BUY" else entry-price)/abs(entry-sl)
def deterministic_active_advice(trade,market,management,review=None):
    direction=_direction(trade.get("direction")); entry=float(trade.get("entry") or 0); price=float(market.get("price") or entry); sl=float(trade["sl"]) if trade.get("sl") is not None else None; tp1=float(trade["tp1"]) if trade.get("tp1") is not None else None; tp2=float(trade["tp2"]) if trade.get("tp2") is not None else None; r=_r_multiple(direction,entry,price,sl); state=str(management.get("state") or "ACTIVE"); lifecycle=str(management.get("decision") or "KEEP"); risk_score=int((review or {}).get("risk_score") or 0); news=market.get("news_decision") or {}
    if news.get("conflict") and news.get("action")=="EXIT": return ActiveTradeLawyerAdvice("EXIT","URGENT",96,"خبر عالي التأثير جاء ضد الصفقة وترافق مع تأكيد سعري معاكس؛ أعد التقييم فوراً.","NEWS_STRESSED",avoid_condition="لا تعزز قبل إعادة التقييم.")
    if news.get("conflict") and news.get("action")=="REDUCE_RISK": return ActiveTradeLawyerAdvice("REDUCE_RISK","HIGH",90,"خبر مؤثر ضد الصفقة؛ خفّض التعرض وراقب تأكيد البنية بدلاً من الخروج الآلي.","NEWS_STRESSED",avoid_condition="لا توسع SL لإنقاذ الصفقة.")
...
def parse_ai(raw):
    ...
    try: return json.loads(str(raw or "").strip().replace("```json","").replace("```","").strip())
...
def should_call_active_ai(last_call,state,previous_state): return bool(previous_state and previous_state!=state) or last_call is None or (time.monotonic()-last_call)>=AI_INTERVAL_SECONDS
...
def _news_context(bot,active_direction=None):
    ...
    price=_price(bot); cache=getattr(bot,"GLOBAL_CACHE",{}); previous=float(cache.get("news_reference_price") or price or 0) if isinstance(cache,dict) else (price or 0); pct=((price-previous)/previous*100) if price and previous else 0.0; price_dir="UP" if pct>0 else "DOWN" if pct<0 else "FLAT"
...
def _install_news_hooks(bot):
    ...
    def wrapped_generate(*args,**kwargs):
        ...
        if active: return original_generate(*args,**kwargs)
        result=original_generate(*args,**kwargs)
        if isinstance(result,dict) and result.get("status")=="SIGNAL": return result
        if os.getenv("NEWS_SIGNAL_ENABLED","1")!="1": return result
        decision=_news_context(bot); candidate=_news_candidate(bot,decision) if decision else None
        if candidate and isinstance(getattr(bot,"GLOBAL_CACHE",None),dict): bot.GLOBAL_CACHE["news_candidate"]=candidate
        return result
    ...
    if original_monitor is not None:
        def wrapped_monitor(*args,**kwargs):
            ...
            try:
                cache=getattr(bot,"GLOBAL_CACHE",{}); thesis=(cache.get("active_trade") or {}) if isinstance(cache,dict) else {}; active_dir=thesis.get("direction")
...
def _news_worker():
    ...
    while True:
        try:
            ...
            if bot:
                _install_news_hooks(bot); decision=_news_context(bot)
                ...
                if app: install_telegram_trade_lawyer(app,bot)
