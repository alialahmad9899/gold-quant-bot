...
_PRICE_HISTORY=deque(maxlen=5000); _EVENTS={}; _LAST_POLL=0.0; _engine=None; _SEMANTIC_PATCHED=False
...
def sample_price(bot):
    p=_price(bot)
    if p and p>0: _PRICE_HISTORY.append((_now(),p))
...
def _event_key(article):
    title=" ".join(str(article.title or "").lower().split()); bucket=int(article.published_at.timestamp()//CLUSTER_SECONDS)
...
def reaction_for(article):
    ...
    baseline=points[0][1]; current=points[-1][1]; change=(current-baseline)/baseline*100 if baseline else 0.0
...
def _ensure_cache(bot):
    ...
    if isinstance(cache,dict): cache.setdefault("news_health",{}); cache.setdefault("news_events",{})

def _patch_semantics():
    global _SEMANTIC_PATCHED
    if _SEMANTIC_PATCHED: return
    import news_intelligence as ni
    from news_semantics import classify
    original=ni.classify_gold_impact
    def wrapped(article): return classify(article,original)
    wrapped._semantic_runtime=True; ni.classify_gold_impact=wrapped; _SEMANTIC_PATCHED=True

def poll_once(bot):
    global _LAST_POLL,_engine
    from news_intelligence import NewsIntelligence, classify_gold_impact
    try:
        _patch_semantics()
        if _engine is None: _engine=NewsIntelligence()
        articles=_engine.fetch_latest(); _LAST_POLL=time.time(); _ensure_cache(bot); cache=bot.GLOBAL_CACHE
        clusters={}; material=[]
        ...
        cache["latest_news"]=[_EVENTS[k] for k in list(_EVENTS.keys())[-20:]]; cache["news_health"]={"last_success":_now().isoformat(),"poll_seconds":POLL_SECONDS,"articles":len(articles),"event_clusters":len(clusters),"confirmed_material":len(material),"reaction_window_seconds":REACTION_WINDOW,"near_real_time":True}
        if material: material.sort(key=lambda x:(x["impact"],x["confidence"]),reverse=True); cache["news_candidate"]=material[0]
        return articles
    except Exception as exc:
        LOGGER.warning("[NEWS_RUNTIME] poll failed: %s",exc); cache=getattr(bot,"GLOBAL_CACHE",None)
...
def health(): return {"installed":_INSTALLED,"last_poll":_LAST_POLL,"events":len(_EVENTS),"price_samples":len(_PRICE_HISTORY),"semantic_classifier":_SEMANTIC_PATCHED}
